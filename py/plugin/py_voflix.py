...
class VoflixRequest(object):

    @staticmethod
    def get(url, params=None, **kwargs):
        res = requests.get(url, params, **kwargs)
        if 'captcha.funcdn' in res.text:
            verify(res)
            print('触发 .....')
...
def calc(challenge, answer):
    for i in range(100000, 1000000):
        if hashlib.md5((challenge + '' + str(i)).encode()).hexdigest() == answer:
            return str(i)


def chech_code(data):
    url = 'https://captcha.funcdn.com/jsCaptchaVerify'
    res = requests.post(url, json=data)
    # print(res.json())
    headers['Cookie'] = '_funcdn_token=' + res.json()['fc_token']

def verify(res):
    user_info = re.findall('userinfo: \'(.*?)\'', res.text)[0]
    host_info = re.findall('hostinfo: \'(.*?)\'', res.text)[0]
    challenge = re.findall('challenge: \'(.*?)\'', res.text)[0]
    answer = re.findall('answer: \'(.*?)\'', res.text)[0]
    code = calc(challenge, answer)
    data = {
        "userinfo": user_info,
        "hostinfo": host_info,
        "challenge": challenge,
...
class Spider(Spider):  # 元类 默认的元类 type
    ...
    def homeVideoContent(self):
        rsp = VoflixRequest.get("https://www.voflix.com/", headers=headers)
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        print(url)
        rsp = VoflixRequest.get(url, headers=headers)
        root = self.html(rsp.text)
    ...
    def detailContent(self, array):
        ...
        vod['vod_content'] = node.xpath(".//div[contains(@class,'module-info-introduction-content')]/p/text()")[
            0].replace('\n', '').replace('\t', '')
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        res = VoflixRequest.get(js_url, headers=headers)
        parseUrl = self.regStr(res.text, 'src="(.*?)\'') + jo['url']
        # parseUrl = 'https://play.shtpin.com/xplay/?url={0}'.format(jo['url'])
        parseRsp = VoflixRequest.get(parseUrl, headers=headers)

        configStr = self.regStr(parseRsp.text, 'var config = ({[\\s\\S]+})')
        configJo = json.loads(configStr)
        playUrl = parseUrl.split('?url=')[
                      0] + '555tZ4pvzHE3BpiO838.php?tm={0}&url={1}&vkey={2}&token={3}&sign=F4penExTGogdt6U8'
        ...
        playRsp = VoflixRequest.get(playUrl.format(time.time(), configJo['url'], configJo['vkey'], configJo['token'])
                             , headers=headers)
    ...
    config = {
        "player": {},
        "filter": {"1": [{"key": "id", "name": "类型",
                          "value": [{"n": "全部", "v": "1"}, {"n": "动作", "v": "6"}, {"n": "喜剧", "v": "7"},
                                    {"n": "爱情", "v": "8"}, {"n": "科幻", "v": "9"}, {"n": "恐怖", "v": "10"},
                                    {"n": "剧情", "v": "11"}, {"n": "战争", "v": "12"}, {"n": "动画", "v": "23"}]},
                         {"key": "class", "name": "剧情",
                   ...
                   "2": [{"key": "id", "name": "类型",
                          "value": [{"n": "全部", "v": "2"}, {"n": "国产剧", "v": "13"}, {"n": "港台剧", "v": "14"},
                                    {"n": "日韩剧", "v": "15"}, {"n": "欧美剧", "v": "16"}, {"n": "纪 录片", "v": "21"},
                                    {"n": "泰国剧", "v": "24"}]}, {"key": "class", "name": "剧情",
                                                                "value": [{"n": "全部", "v": ""}, {"n": "古装", "v": "古装"},
                                                                          {"n": "战争", "v": "战争"},
                                                                          {"n": "青春偶像", "v": "青春偶像"},
                                                                          {"n": "喜剧", "v": "喜剧"},
                                                                          {"n": "家庭", "v": "家庭"},
                                                                          ...
                                                                          {"n": "其他", "v": "其他"}]},
                         {"key": "area", "name": "地区",
                          "value": [{"n": "全部", "v": ""}, {"n": "中国大陆", "v": "中国大陆"}, {"n": "中国台湾", "v": "中国台湾"},
                                    {"n": "中国香港", "v": "中国香港"}, {"n": "韩国", "v": "韩国"}, {"n": "日本", "v": "日本"},
                                    {"n": "美国", "v": "美国"}, {"n": "泰国", "v": "泰国"}, {"n": "英国", "v": "英国"},
                                    {"n": "新加坡", "v": "新加坡"}, {"n": "其他", "v": "其他"}]}, {"key": "year", "name": "年份",
                                                                                         "value": [{"n": "全部", "v": ""},
                                                                                                   {"n": "2022",
                                                                                                    "v": "2022"},
                                                                                                   {"n": "2021",
                   ...
                   "4": [{"key": "class", "name": "剧情",
                         ...
                         {"key": "area", "name": "地区",
                          "value": [{"n": "全部", "v": ""}, {"n": "中国", "v": "中国"}, {"n": "日本", "v": "日本"},
                                    {"n": "欧美", "v": "欧美"}, {"n": "其他", "v": "其他"}]}, {"key": "year", "name": "年份",
                                                                                       "value": [{"n": "全部", "v": ""},
                                                                                                 {"n": "2022",
                                                                                                  "v": "2022"},
                                                                                                 {"n": "2021",
